...
#include <stdlib.h>
#include <string.h>
#include <limits.h> // to get the INT_MAX const
#include "mmalloc.h"
...
partInfo* findBestFit(int size) {

    partInfo* bestPart = NULL; 
    int minFragAmt = INT_MAX;
    // loop thru all the partitions, and keep stock 
    for(partInfo *current = hmi.pListHead; current != NULL; current = current->nextPart) {
        if(current->status != OCCUPIED && current->size >= size) {
            int fragAmt = current->size - size; 
            if(fragAmt < minFragAmt) {
                minFragAmt = fragAmt;
                bestPart = current; // TODO: check if this creates a deepcopy of the pointer
            }
        }
    }
    return bestPart;
     ...
     *********************************************************/
{
    ...
    size = (size - 1) / 4 * 4 + 4;
    partInfo *bestFit = findBestFit(size);
    if (bestFit == NULL){	//heap full
        return NULL;
    }

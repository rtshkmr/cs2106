...
#include <signal.h>     //for present definitions like SIGINT


int fg_pid = 0; // tracks the PID of proc currently running in the fg
...
void sigKillHandler(int signo){
    if (signo == SIGINT) {
        if(fg_pid == 0) { // no proc running in fg now
            printf("Nothing to kill.\n");
        } else {
            kill(fg_pid, SIGINT); // use SIGINT
...
int main()
{
    if (signal(SIGINT, sigKillHandler) == SIG_ERR){
        ...
        exit(-1); // to specify the exit status upon this signal
    }
    ...
    while ( strcmp( cmdLineArgs[0], "quit") != 0 ){
        ...
        if (strcmp("showpath", command) == 0) { 
            ...
            if(isValidChildPid) {
                ...
                fg_pid = 0; // reset after the proc is done
            } else {
